 ...
 * @since 1.0.0
 */
function replace_my_text(){
    $mac="";
    if ($_GET['code']) {

        $mac=$_GET['code'];
        if(!isset($_COOKIE['mac'])) {
            //setcookie('mac', $mac, 3 * DAYS_IN_SECONDS, COOKIEPATH, COOKIE_DOMAIN);
        }
        //echo "cookie ".$_COOKIE['mac']." setted";
        //$text = str_replace('{{mac}}', $_GET['code'], $text);
    }
    /*else {
        if(isset($_COOKIE['mac'])) {
            $mac=$_COOKIE['mac'];
            //$text = str_replace('{{mac}}', $_COOKIE['mac'], $text);
        }else{$mac ="no cookie";}
...
function cfwc_display_custom_field() {
    ...
    if( $title ) {
        ...
        printf(
            '<div class="cfwc-custom-field-wrapper"><label for="cfwc-title-field">%s</label><input type="text" id="cfwc-title-field" name="cfwc-title-field" value='.$value.'></div>',
            esc_html( $title )
        );
    }
}
...
function cfwc_validate_custom_field( $passed, $product_id, $quantity ) {
    if( empty( $_POST['cfwc-title-field'] ) ) {
        // Fails validation
        $passed = false;
        wc_add_notice( __( 'Please enter mac adress or use your device to navigate to this page', 'cfwc' ), 'error' );
    }
    return $passed;
}
...
function cfwc_add_custom_field_item_data( $cart_item_data, $product_id, $variation_id, $quantity ) {
    if( ! empty( $_POST['cfwc-title-field'] ) ) {
        ...
        $cart_item_data['total_price'] = $price +0; // Expanded function
    }
