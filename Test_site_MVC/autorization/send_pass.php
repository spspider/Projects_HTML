...
if (isset($login) &&(!isset($myrow_a['id'])) ) {//если существуют необходимые переменные

	include ("bd.php");// файл bd.php должен быть в той же папке, что и все остальные, если это не так, то просто измените путь

	$result = mysql_query("SELECT id FROM users WHERE login='$login' AND activation='1'",$db);//такой ли у пользователя е-мейл
...
mysql_query("UPDATE users SET password='$new_password_sh' WHERE login='$login'",$db);// обновили в базе
	...
	mail($login, "Восстановление пароля", $message, "Content-type:text/plane; Charset=windows-1251\r\n");//отправляем сообщение
